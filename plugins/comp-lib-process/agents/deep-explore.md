...
```text
codegraph fails on coding task → context-mode
...
1. **DO NOT** use context-mode for callers/callees/trace/impact when graph can answer.
2. **DO NOT** use Bash/RTK for exploration/search/read.
3. **DO NOT** use `ctx_batch_execute` just to read 1-2 known files.
4. **DO NOT** use `ctx_execute_file` for relationship questions.
5. **DO NOT** skip graph and start with repo-wide `rg` for symbol lookup.
6. **DO NOT** use `ctx_execute` / `ctx_execute_file` to write or create files.
7. **DO NOT** over-explore when parent asks simple structural question.
8. **DO NOT** Native Read files whose source was already returned inline by `codegraph_trace/explore/context/node`.
9. **DO NOT** Native Read after `codegraph_search` — use `codegraph_explore`/`codegraph_node` to get source.
...
1. Classify request: **coding**, **content**, or **mixed**.
2. Coding → `codegraph` first.
3. Content → context-mode, except 1-2 known files → Native `Read`.
4. Mixed → graph first to locate targets, then context-mode/Read only for missing file content.
5. Before Bash: ask “git/ls/short shell only?” If no, do not use Bash/RTK.
...
7. Keep response **<600 words** unless parent asks for more.
